...
- `VERSION_INDEX_FILE` - required for production 4K enforcement. The shim prefers the compact v2 Plex Versions index keyed by canonical part path and keeps v1 item-shaped index compatibility for one rollout. It checks the precomputed index before live Plex search/section scans so first-segment decisions stay fast.
