...
- Bragi Live TV/DVR/tuner/HDHomeRun/Dispatcharr streams are outside Downshiftarr's Plex Versions waterfall model. `Downshiftarr.py` must bypass explicit Live TV events before Plex lookup, and the `Plex Transcoder` shim must bypass Live TV/tuner invocations before cache, v2 locator, or Plex API lookup.
...
## 2026-06-01 Shim Parser And Argument-Rewrite Hardening Campaign

- Q: Which native fuzz target should run first? A: Run `shim-parsers` first because the current risk is the Plex Transcoder parser and argument-rewrite hot path.
- Q: How should multiple video-looking `-i` inputs behave? A: Keep the current last-video-input selection behavior for this campaign and lock it with regression coverage.
- Q: What initial native fuzz budget should be used? A: Use the extended first pass, `--runs 100000 --max-total-time 300`, before final closeout.
- Q: How should manual hardening commands invoke `uv` in this Windows-backed WSL checkout? A: Use `UV_LINK_MODE=copy uv run --locked` for the outer project command; the native Atheris runner remains responsible for launching the fuzz target with isolated Python 3.11.
- Campaign evidence: WSL host `Linux Loki 6.6.87.2-microsoft-standard-WSL2`, branch `main...origin/main`, base commit `f609eac`, `uv 0.11.16`, project Python `3.12.13`, native fuzz Python `3.11.15`. Pre-fix `shim-parsers` campaign passed with `100000` runs and no crash artifacts. After parser/rewrite fixes, the final command `UV_LINK_MODE=copy DOWNSHIFTARR_HARDENING_MANUAL=1 uv run --locked python scripts/testing/run_native_fuzz.py --target shim-parsers --runs 100000 --max-total-time 300 --run` passed with seed `2670854876`, `100000` runs, exit code `0`, and no files under `artifacts/hardening/atheris/`.
- Implementation note: parser hardening now ignores streaming and tonemap words when they appear only in filenames, counts required stream indices only from selector/filter arguments, preserves current last-video-input behavior, avoids consuming the next option when malformed bitrate flags are missing values, and preserves `zscale` resize filters including `size=` while still removing tone-map filters after SDR swaps.
- Regression coverage added for filename false positives, stream-reference parsing, tonemap detection, last video input selection, malformed bitrate option handling, output codec preservation, `zscale=size` preservation, and expanded native fuzz coverage for streaming, tonemap, and cache-authoritative fallback helpers. No real Loki/Plex/Tautulli/browser/Docker/GitHub proof was part of this campaign; all artifacts stayed local/ignored and no crash/minimized inputs were produced.
