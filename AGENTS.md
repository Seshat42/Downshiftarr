...
- Default transcode behavior must preserve Plex UX while reducing server load: any source with actual height above 1080 (`PROTECTED_SOURCE_MIN_HEIGHT=1081`) is protected and must downshift or block, while 1080 HDR/remux-like sources waterfall by default and do not hard-block solely because no lower fallback exists. The default 1080 remux-like threshold is `REMUX_1080_MIN_BITRATE_KBPS=25000`.
...
- The Plex Transcoder shim must never allow a protected `>1080` source to transcode. It uses the compact v2 index only as an exact path-to-`ratingKey` locator, treats every non-v2 index as invalid, and must fetch `/library/metadata/{ratingKey}` as the authoritative Plex Versions source before any swap. If the locator is missing, stale, or misses, it may attempt one bounded exact-section Plex lookup inside the 100 ms decision budget. Protected metadata lookup gets one immediate retry inside that same budget, then blocks with a brief retry/media-readiness message if no verified lower same-item/same-edition version is proven. Generic fallback cache entries may be used only after fresh actual-height proof shows the source is not protected.
- Shadow mode is observation-only for non-4K decisions, but it must be overridden for 4K because `FOUR_K_TRANSCODE_ALLOWED=0` is a hard production invariant.
- The Plex Transcoder shim has a 100 ms default decision budget. Cap Plex API timeouts to the remaining budget and pass through non-4K decisions rather than making Plex clients wait when confidence or timing fails, including when strict unsure-kill policy would otherwise apply; 4K transcode attempts still block unless a fresh index proves a lower Plex Version.
- Broad evaluation should use shadow mode plus sanitized aggregate telemetry first. Plex Transcoder shim telemetry may contain only outcome counters, version-index status, latency percentiles, and no-fallback diagnostics; device/client-family classification must stay out of the shim hot path. Downshiftarr/Tautulli controller telemetry may contain client-family buckets and adaptive-rule confidence only; never store raw usernames, tokens, IPs, rating keys, session ids, machine identifiers, or watch timelines.
- Adaptive fallback learning is disabled for this pass. Historical state helpers may remain inert for compatibility, but deterministic fixed fallback order is authoritative.
