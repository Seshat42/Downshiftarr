...
- Git remote storage metadata for `origin/main`.
...
- Local sidecar API key: generated by isolated Tautulli and stored only in ignored `Downshiftarr.test.env`.
...
- GitHub is remote Git storage only. Hosted automation is not an accepted trust boundary for this repo.
...
- If the isolated sidecar loses Docker-to-Loki reachability, local Tautulli proof can pass web health while failing Plex monitoring.
...
- A future hosted or packaging process accidentally includes ignored local files, proof logs, generated media, screenshots, or env files. Local artifact hygiene and secret scans must fail before anything is pushed or distributed.
