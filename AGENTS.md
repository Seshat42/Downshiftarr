...
- Default transcode behavior must preserve Plex UX while reducing server load: any source with actual height above 1080 (`PROTECTED_SOURCE_MIN_HEIGHT=1081`) is protected and must downshift or block, while 1080 HDR/remux sources waterfall by default and do not hard-block solely because no lower fallback exists.
