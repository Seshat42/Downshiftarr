...
- The Plex Transcoder shim must never allow a protected `>1080` source to transcode. It uses the compact v2 index only as an exact path-to-`ratingKey` locator, rejects older v1 item-shaped indexes, and must fetch `/library/metadata/{ratingKey}` as the authoritative Plex Versions source before any swap. If the locator is missing, stale, or misses, it may attempt one bounded exact-section Plex lookup inside the 100 ms decision budget. Generic fallback cache entries may be used only after fresh actual-height proof shows the source is not protected. If no verified lower same-item/same-edition version is proven before the budget expires, it blocks the protected transcode with a brief retry/media-readiness message.
