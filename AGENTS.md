...
- The Plex Transcoder shim must never allow a 4K source to transcode. For 4K paths, it must use a fresh precomputed Plex Versions index before cache or live lookup; if no verified lower same-item/same-edition version is available, it blocks the transcode. `ALLOW_LIVE_LOOKUP_ON_INDEX_MISS=1` is allowed only for non-4K continued-transcode diagnostics.
